...
export default function EmployeeHours({
  ...
  const presenceDays = new Set([
    ...monthRows.map(row => isoDate(row.work_jobs?.work_date)).filter(Boolean),
    ...selfLogs
      .map(item => isoDate(item.work_date))
      .filter(date => {
        const dt = DateTime.fromISO(date, { zone: 'Europe/Berlin' })
        return dt.isValid && dt.year === month.year && dt.month === month.month
      }),
  ]).size
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        {t('hoursHint') ? <p className="mt-2 text-sm text-slate-300">{t('hoursHint')}</p> : null}
      </div>
