...
BEGIN
  ...
  IF OLD.worker_id = public.current_worker_id() THEN
  ...
  IF NOT public.worker_shares_job(OLD.job_id) THEN
    RAISE EXCEPTION 'not allowed';
  END IF;
  IF NEW.status IS DISTINCT FROM OLD.status
     OR NEW.seen_at IS DISTINCT FROM OLD.seen_at
...
CREATE POLICY "Work assignees update crew"
  ON public.work_job_assignees
  FOR UPDATE
  TO authenticated
  USING (
